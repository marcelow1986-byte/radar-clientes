...
async function carregarRotas() {
  ...
  if (erroItens) {
    alert("Falha ao carregar clientes das rotas: " + erroItens.message);
    return;
  }

  const rotasComTotais = (rotasData || []).map((rota) => {
    const itens = (itensRota || []).filter(
      (item) => item.rota_id === rota.id
    );
      ...
      {(telaAtual === "clientes" ||
        ...
        <section className="painel">
          ...
          <div className="acoes-filtro">
            ...
            <button
              type="button"
              onClick={buscarClientesProximos}
            >
